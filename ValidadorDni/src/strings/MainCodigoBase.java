...
public class MainCodigoBase {
	...
	public static void main(String[] args) {
		// Expresión regular para validar DNI OR NIE
		
		String regex = "(^\\d{8}[A-Z&&[^IÃ‘OU]])|(^[XYZ]\\d{7}[A-Z&&[^IÃ‘OU]])";
		
		/* Caso test básico: debe mostrar el mensaje en consola:
		 * "No se ha encotrado el patrón en " 
		 * o PASS
		 * */
		...
		// Compilamos la expresión regular (creamos el patrón)
		
		Pattern patron = Pattern.compile(regex);
		
		// Creamos el objeto matcher donde buscar la expresión regular (el patrón) 
		
		Matcher matcher = patron.matcher(dni);
		
		System.out.println( matcher );
		
		// find() devuelve true si encuentra el patrón en el matcher.
		
		if(matcher.find()){
				 System.out.println( dni + " PASS");
		}
		else System.out.println( "No se ha encotrado el patrón en " + dni); 
		...
		for(String caso : casosTest){
			...
			if(match.find()){
				 ...
				 // group() extrae el string que satisface la expresión regular del patrón  
				
				 String cadenaMatchingRegex = match.group();
				 
				 // Si el string que ha hecho matching con la expresión regular es igual al caso test =>
			...
			else System.out.println( "No se ha encotrado el patrón en " + caso ); 
		...
		// generamos los casos test y los añadimos al ArrayList
